...
public abstract class TraitementMatieres {
    public static void TraitementMatiere(){
        ...
        System.out.println("Size eventTraite : "+_eventsTraites.size());
        if(GetEvents._listeMatieresAEnlever.isEmpty()){
            ...
            for(String matiere : GetEvents._listeMatieresAEnlever){
                ...
                for(int i=0;i<_eventsTraites.size();i++){
                    System.out.println(_eventsTraites.get(i).get_matiere()!=null && _eventsTraites.get(i).get_matiere().equalsIgnoreCase(matiere));
